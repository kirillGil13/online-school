<template>
  <v-col class="pa-6 d-flex justify-center flex-column status_form">
      ...
      <v-radio-group row hide-details v-model="form[attrs.name]" v-bind="attrs" class="status_radio">
        ...
        >
          <template v-slot:label>
            <v-img class="status_img" :src="icon.fullLink" :width="$adaptive.isMobile ? 32 : 24"></v-img>
          </template>
        </v-radio>
      </v-radio-group>
    </FormGroup>
    ...
    <div class="d-flex flex-row justify-space-between mt-2">
      <Button class="mr-3" full-width small :disabled="form.disabled" @submit="$emit('createStatus')">Добавить статус</Button>
