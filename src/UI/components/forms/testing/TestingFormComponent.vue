<template>
  <v-col class="pa-0">
    <form @submit.prevent>
      <ProgressBar :amount="form.questions.length" :step="step" :active-step="form.activeStep"/>
      <TestingQuestion
      ...
      <v-row class="ma-0 justify-end">
        ...
        <Button v-if="step === form.questions.length" :disabled="!form.valid(step)" @submit="$emit('send')"
        >Отправить задание
        </Button>
      </v-row>
    </form>
...
export default class TestingFormVue extends Vue {
  ...
  next(): void {
    this.step += 1;
    this.form.activeStep[this.step - 1].active = true;
  }

  previous(): void {
    this.step -= 1;
    this.form.activeStep[this.step].active = false;
  }
